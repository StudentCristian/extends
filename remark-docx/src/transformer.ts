...
export const mdastToDocx = async (
    node: mdast.Root,
    { output = "buffer", title, subject, creator, keywords, description, lastModifiedBy, revision, styles, background }: DocxOptions,
    images: ImageDataMap,
): Promise<any> => {
...
const getOrCreateFootnoteId = (identifier: string, registry: FootnoteRegistry): number => {
...
const buildList = ({ children, ordered, start: _start, spread: _spread }: mdast.List, ctx: Context) => {
    const list: ListInfo = {
...
const buildListItem = ({ children, checked, spread: _spread }: mdast.ListItem, ctx: Context) => {
    const { nodes, footnotes: nestedFootnotes } = convertNodes(children, {
        ...ctx,
...
const buildTableRow = ({ children }: mdast.TableRow, ctx: Context, cellAligns: AlignmentType[] | undefined) => {
    return new TableRow({
...
const buildTableCell = ({ children }: mdast.TableCell, ctx: Context, align: AlignmentType | undefined) => {
...
const buildLink = ({ children, url, title: _title }: mdast.Link, ctx: Context) => {
    const { nodes, footnotes: nestedFootnotes } = convertNodes(children, ctx);
    return {
...
const buildImage = ({ url, title: _title, alt: _alt }: mdast.Image, images: ImageDataMap) => {
...
const buildFootnote = ({ children }: mdast.Footnote, ctx: Context): { footnoteRef: FootnoteReferenceRun; footnoteData: Footnotes } => {
